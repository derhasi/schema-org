...
class Filesystem
{
    ...
    public function cloneStaticFiles()
    {
        ...
        foreach ($files as $file) {
            ...
            $this->flysystem->put(str_replace('generator/templates/static', 'src', $file['path']), $this->flysystem->read($file['path']));
        }
    }
    public function createType(Type $type)
    {
        $this->flysystem->put("src/{$type->name}.php", $this->typeTemplate->render(['type' => $type]));
    }
    public function createFactory(TypeCollection $types)
    {
        $this->flysystem->put('src/Schema.php', $this->factoryTemplate->render(['types' => $types->toArray()]));
    }
}